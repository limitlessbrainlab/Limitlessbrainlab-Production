...
const InteractiveBrainHero: React.FC<InteractiveBrainHeroProps> = ({ targetLobe }) => {
  ...
  useEffect(() => {
    ...
    import('lodash').then((_) => {
      ...
      // Get all brain functions and shuffle them
      const brainFunctions = _.shuffle(_.uniq(_.flatten(_.map(info.brain, 'highlight'))));

      let functionsItems = '';
      _.each(brainFunctions, (func: string) => {
        functionsItems += `<div class="${CLASSES.filter_item} item-${func}">${keyToString(func)}</div>`;
      });

      const filterContainer = componentRef.current.querySelector(SELECTORS.filter_container);
      if (filterContainer) {
        filterContainer.innerHTML = functionsItems;
      }

      // Set up hover interactions for each brain part
      _.each(info.brain, (part, k) => {
        const brainPart = componentRef.current?.querySelector(part.selector);
        if (brainPart) {
          const mouseEnterHandler = () => {
            const infoTitle = componentRef.current?.querySelector(SELECTORS.info_title);
            const infoDescription = componentRef.current?.querySelector(SELECTORS.info_description);
            
            if (infoTitle) infoTitle.innerHTML = part.title;
            if (infoDescription) infoDescription.innerHTML = part.description;

            _.each(part.highlight, (func: string) => {
              const item = componentRef.current?.querySelector(`.item-${func}`);
              if (item) {
...
export default InteractiveBrainHero;